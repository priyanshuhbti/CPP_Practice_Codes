...
int main()
{
    ...
    while (testCases--) {
        int size;
        cin >> size; // Read the size of the permutations

        vector<int> permutation(size);
        // Initialize permutation with 1 to size
        ...
        int middle = (size + 1) / 2;
        vector<int> newPermutation(permutation.begin() + middle, permutation.end());
        newPermutation.insert(newPermutation.end(), permutation.begin(), permutation.begin() + middle);

        // Output the rearranged permutation
        for (const auto& num : newPermutation) {
            cout << num << ' ';
        }
        cout << '\n';
    }
